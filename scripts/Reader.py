...
class Pn532Reader:
    def __init__(self):
        pn532 = Pn532_i2c()
        logger.info('PN532 i2c communication initialized')
        self.device = Mifare()
        logger.info('MiFare configured')
        ...
        logger.info('PN532 initialization complete')
    def readCard(self):
        cardid = self.device.scan_field()
        if cardid == False:
            logger.debug('no card')
            return None
        return str(+int('0x' + cardid.hex(), 0))
    ...
    def __init__(self):
        ...
        if not os.path.isfile(path + '/deviceName.txt'):
            ...
            if device_name == 'MFRC522':
                ...
                logger.info('initializing PN532 device')
                self.reader = Pn532Reader()
                logger.info('PN532 initialized')
            else:
                try:
