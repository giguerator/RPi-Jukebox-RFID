...
class DisplayPIL(Display):
    ...
    def __init__(self, args=None):
        ...
        self.icons_battery_full = Image.open(
                os.path.join(self._image_dir, "battery_full.png")).resize(target_size, resample=Image.LANCZOS).convert("RGBA")
        self.icons_battery_75 = Image.open(
                os.path.join(self._image_dir, "battery_75.png")).resize(target_size, resample=Image.LANCZOS).convert("RGBA")
        self.icons_battery_50 = Image.open(
                os.path.join(self._image_dir, "battery_50.png")).resize(target_size, resample=Image.LANCZOS).convert("RGBA")
        self.icons_battery_25 = Image.open(
                os.path.join(self._image_dir, "battery_25.png")).resize(target_size, resample=Image.LANCZOS).convert("RGBA")
        self.icons_battery_empty = Image.open(
                os.path.join(self._image_dir, "battery_empty.png")).resize(target_size, resample=Image.LANCZOS).convert("RGBA")
    ...
    def update_overlay(self, *args):
        ...
        if (self._last_title, self._last_artist, self._last_album, self._last_charge) != (self._title, self._artist, self._album, self._charge):
    ...
    def update_text_layer(self):
        ...
        box = text_in_rect(self._text_draw, artist, self._font_medium, (margin, 60 * self._downscale, width - margin, 80 * self._downscale))

        # Album
        #text_in_rect(self._text_draw, "%s" % self._charge, self._font_small, (50 * self._downscale, box[3], width - (50 * self._downscale), 70 * self._downscale))
    ...
    def redraw(self):
        ...
        if self._output_image is not None \
                ...
                and self._last_charge == self._charge:
        ...
        self._last_charge = self._charge
        ...
        # Overlay charge info icon + text
        if self._charge > 80:
            overlay = Image.alpha_composite(overlay, self.icons_battery_full)
        elif self._charge > 55:
            overlay = Image.alpha_composite(overlay, self.icons_battery_75)
        elif self._charge > 30:
            overlay = Image.alpha_composite(overlay, self.icons_battery_50)
        elif self._charge > 5:
            overlay = Image.alpha_composite(overlay, self.icons_battery_25)
        else:
            overlay = Image.alpha_composite(overlay, self.icons_battery_empty)
